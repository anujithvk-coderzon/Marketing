...
}).refine((data) => !data.saveAsTemplate || (data.saveAsTemplate && data.name), { message: "Template name required when saving as template", path: ["name"] });
exports.campaignCreationValidation = zod_1.z.object({
    name: zod_1.z.string().min(1, "Campaign name required")
});
const emptyToUndefined = zod_1.z.preprocess((val) => val === '' ? undefined : val, zod_1.z.string().optional());
exports.emailEditValidation = zod_1.z.object({
    name: zod_1.z.preprocess((val) => val === '' ? undefined : val, zod_1.z.string().min(1).optional()),
    email: zod_1.z.email("Invalid email address").min(1, "Email required"),
    phone: zod_1.z.preprocess((val) => val === '' ? undefined : val, zod_1.z.string().regex(/^\d{10}$/, "Invalid phone number").optional()),
...
}).refine((data) => !data.scheduledAt || (data.templateId && data.recipients && data.recipients.length > 0), { message: "Template and at least one recipient are required to schedule a campaign" });
